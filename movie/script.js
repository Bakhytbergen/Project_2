...
/* In this function, I get the user's 
query from the input field. If the query is empty, 
I hide the autocomplete list. If there is a request, 
I send a request to the API to search for movies and process the received data:
 save the results to localStorage, display the movies on the page and show the autocomplete with the search results*/
...
/*
 In this function, I clear the current movie cards on the page and then create new cards
  for each movie from the array passed to the function. Each card includes information about 
  the movie, image, title, release year, and buttons to view details and add to favorites. 
  I check to see if the movie is in the favorites list and change the color of the add to favorites button depending on that
*/
...
/*
In this function, I handle the display of autocomplete suggestions for 
the search field. When the user types, I send a request and display a 
list of suggestions that can be selected. If the list is empty, it’s hidden.
 If an item is selected, the input field is filled with the chosen movie title, and a new search is triggered.
 */
...
/*
 In this function, I implement sorting of search results 
 based on different criteria (popularity, release date, rating).
  I retrieve the search results from localStorage, sort them according to the 
  selected criterion, and display the sorted results on the page.
 */
...
/*
When the user clicks on the "View Details" 
button, I send a request to get detailed information about the movie 
(including actors, trailers, and additional data). After receiving the data,
 I create HTML for the modal window, display the movie details, and show the actor list. 
 If a trailer is available, I show a button to link to YouTube.
*/
...
/*
: In this function, I handle adding or 
removing a movie from the watchlist. When the "Add to Watchlist"
 button is clicked, I check if the movie is already in the watchlist and either add it or remove it.
...
//In this function, I display the list of movies added to my “favorites” (watchlist) on the page
